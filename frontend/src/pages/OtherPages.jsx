    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface border border-border rounded-xl px-4 py-3 inline-flex gap-6 text-sm">
        ...
        <div>
          ...
          <span className="font-mono font-medium">{list.length}</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Empty icon="📈" message="Nenhum investimento cadastrado" />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((i) => (
            <Card
              key={i.id}
              accent={typeColor[i.type] || "#888"}
              className="p-5"
            >
              ...
              <div
                ...
                style={{ color: typeColor[i.type] || "#888" }}
              >
                {fmtBRL(i.amount)}
              </div>
        ...
        <div className="flex flex-col gap-4">
          <FormField label="Nome">
            <input
              ...
              placeholder="Tesouro Direto, CDB..."
            />
          </FormField>
...
export function Goals() {
  ...
  const deposit = async () => {
    ...
    try {
      await api.post(`/goals/${depositModal}/deposit`, {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <Empty icon="🎯" message="Nenhuma meta cadastrada" />
      ) : (
              ...
              <Card key={g.id} className="p-5">
                    ...
                    <div>
                      ...
                      {g.deadline && (
                        <div className="text-xs text-gray-500 mt-0.5">
                          até {g.deadline.split("-").reverse().join("/")}
                        </div>
                      )}
                      {g.isCompleted && (
                ...
                <div className="flex justify-between text-xs text-gray-500 mb-4">
                  ...
                  <span
                    ...
                    style={{ color: pct >= 100 ? "#4ade80" : "#a78bfa" }}
                  >
                    {pct}%
                  </span>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => setDepositModal(null)} variant="ghost">
              Cancelar
            </Button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {list.some((a) => !a.isRead) && (
          <Button onClick={markAll} variant="ghost" className="text-xs">
      ...
      {loading ? (
        ...
        <Empty icon="🔔" message="Nenhum alerta por enquanto" />
      ) : (
        <div className="flex flex-col gap-3">
          {list.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className={`rounded-xl border p-4 flex items-start gap-4 transition-opacity ${s.border} ${s.bg} ${a.isRead ? "opacity-50" : ""}`}
              >
                <div
                  className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${a.isRead ? "bg-gray-600" : s.dot}`}
                />
                <div className="flex-1">
                  <div className={`font-semibold text-sm ${s.text}`}>
                    {a.title}
                  </div>
