  ...
  const load = useCallback(async () => {
    ...
    try {
      const [eRes, cRes] = await Promise.all([
        api.get("/expenses", {
          params: { year, month, categoryId: catFilter || undefined },
        }),
        api.get("/categories"),
      ]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        {loading ? (
          ...
          <Empty icon="📭" message="Nenhum gasto lançado ainda" />
        ) : (
          <table className="w-full">
            ...
            <tbody>
              {expenses.map((e) => (
                <tr
                  key={e.id}
                  className="border-t border-border hover:bg-surface2/50 transition-colors"
                >
                  ...
                  <td className="px-4 py-3.5 font-mono text-sm text-red-400">
                    {fmtBRL(e.amount)}
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-xs">
                    {e.isRecurring && (
                      <span className="text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-md">
                        Fixo
                      </span>
                    )}
                  </td>
